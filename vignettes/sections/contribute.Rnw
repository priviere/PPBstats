...
\item create the \emph{fit\_my\_model} method for \texttt{check\_model()}
<<message=FALSE,cache=FALSE>>=
...
\item create the \emph{check\_my\_model} method for \texttt{mean\_comparisons()}
...
\item create the \emph{mean\_comparisons\_my\_model} method for \texttt{plot()}
