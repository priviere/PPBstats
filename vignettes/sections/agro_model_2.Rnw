...
Note that you can highlight some individual with the argument \texttt{ind\_to\_highlight}.
...
\item \texttt{clust} : output from \texttt{factextra::fviz\_cluster()}, a list of number of cluster + 1 element
  <<message=TRUE,cache=FALSE>>=
  cl = ppg$clust
  names(cl)
  @
...
\texttt{ptp} can be handle exacly as \texttt{out\_model\_1} for mean comparisons

<<message=TRUE,cache=FALSE>>=
m_ptp = mean_comparisons(ptp)
@

and visualize the output, for \textttt{ggplot.type = "barplot"}, there is two colors regarding estimated and predicted results.
...
For \textttt{ggplot.type = "score"} and \textttt{ggplot.type = "interaction"}, predicted and estimated are mention between brakets after germplasm name.
...
If you wish to apply the model\_2 workflow to several variables, you can use \texttt{lapply} with the following code :
