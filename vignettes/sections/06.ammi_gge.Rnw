...
ammi = GxE(d, vec_variables, gxe_analysis = "AMMI")
@
...
<<message=TRUE,cache=FALSE>>=
res_ammi_y1$model$anova_model
@


<<message=TRUE,cache=FALSE>>=
res_ammi_y1$model$residuals
@


<<message=TRUE,cache=FALSE>>=
res_ammi_y1$model$variability_repartition
@


\subsubsection{Get main effects results}
<<message=TRUE,cache=FALSE>>=
res_ammi_y1$germplasm
@


\paragraph{Germplasm effects}
<<message=TRUE,cache=FALSE>>=
res_ammi_y1$location
@
...
<<message=TRUE,cache=FALSE>>=
res_ammi_y1$GxE$interaction_plot
@
...
<<message=TRUE,cache=FALSE>>=
res_ammi_y1$GxE$ecovalence
@


\paragraph{PCA}
The results of the PCA is a list divided into three elements as explained in \citet{ceccarelli_technical_2015}:
...
<<message=TRUE,cache=FALSE>>=
res_ammi_y1$GxE$PCA$variation_dim
@
...
<<message=TRUE,cache=FALSE>>=
res_ammi_y1$GxE$PCA$which_won_where
@
...
<<message=TRUE,cache=FALSE>>=
res_ammi_y1$GxE$PCA$mean_vs_stability$mean
@
...
<<message=TRUE,cache=FALSE>>=
res_ammi_y1$GxE$PCA$mean_vs_stability$stability
@
...
<<message=TRUE,cache=FALSE>>=
res_ammi_y1$GxE$PCA$discrimitiveness_vs_representativeness$discrimitiveness
@
...
<<message=TRUE,cache=FALSE>>=
res_ammi_y1$GxE$PCA$discrimitiveness_vs_representativeness$representativeness
@
\end{itemize}
