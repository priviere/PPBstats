...
\item Family 4 gathers analysis answering specific research questions. This is for analysis on one farm or more.
\end{itemize}
...
2 & AMMI & \ref{ammi} & & X & X & (X) & X & (X) & & \\
  ...
  & Bayesian model 2 & \ref{bayesian_model_2} & & X & X & (X) & X & & & \\
\hline
3 & Bayesian model 3 & \ref{bayesian_model_3} & X & X & X & & X & X & & \\
\hline
...
\noindent Function \texttt{plan\_experiment} settle experimental design based on :
\begin{itemize} 
...
\noindent A description of the algorithme is describe in the help of the function: \texttt{?plan\_experiment}.
...
p_fr = plan_experiment(
  expe.type = "fully-replicated", 
  entries = paste("germ", c(1:20), sep = ":"), 
  nb.blocks = 3, 
  nb.cols = 4)
@
...
You can set the format to a SHiNeMaS\footnote{} reproduction template file:
...
<<message=TRUE,cache=FALSE>>=
p_ibd = plan_experiment(
  expe.type = "IBD", 
  entries = paste("germ", c(1:10), sep = ":"),
  nb.blocks = 8, 
  nb.cols = 4)
@
