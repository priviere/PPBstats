...
\begin{itemize}
  ...
  \begin{itemize}
    \item A presence.absence plot. Note that it may be different from experimental design planned because of NA.
    The plot represents the presence/absence matrix of G $\times$ E combinaisons. 
    <<message=TRUE,cache=FALSE>>=
    describe_data_GxE$each_variable$y1$presence.absence
    @
    A score of 3 is for a given germplasm replicated three times in a given environement.
    
    <<message=TRUE,cache=FALSE>>=
    describe_data_model_2$each_variable$y1$presence.absence
    @
