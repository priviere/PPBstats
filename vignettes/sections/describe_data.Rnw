...
describe_data_GxE = describe_data(data_GxE, vec_variables = c("y1"))
describe_data_model_2 = describe_data(data_model_2, vec_variables = c("y1"))
@
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item A list with histogram. Note that for each element of the following list, there are as many graph as needed with \code{nb_parameters_per_plot} parameters per graph
    \begin{itemize}
      \item germplasm
      <<message=TRUE,cache=FALSE>>=
    ...
    \item A list with boxplot, containg a list with plot and outliers. Note that for each element of the following list, there are as many graph as needed with \code{nb_parameters_per_plot} parameters per graph
    \begin{itemize}
      \item germplasm
      <<message=TRUE,cache=FALSE>>=
    ...
    \item interaction. Note that for each element of the following list, there are as many graph as needed with \code{nb_parameters_per_plot} parameters per graph
    <<message=TRUE,cache=FALSE>>=
