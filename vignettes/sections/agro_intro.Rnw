...
\rotatebox{90}{variance intra population effects} & 
\\
\hline
...
4 & Version & \ref{version} & X & (X) & (X) & (X) & (X) & (X) & & X & \\
  & Variance intra & \ref{variance_intra} & & & & & & & & & X \\
\hline
\end{tabular}
...
\begin{figure}[H]
\begin{center}
\input{figures/PPBstats_main_functions_agro.tex}
\end{center}
...
\texttt{spatial} & Run spatial row and column model \\ \hline

\texttt{model\_variance\_intra} & Run model variance intra \\ \hline

\texttt{multivariate} & Run multivariate analysis with function from \texttt{FactoMineR} \\ \hline
...
\texttt{data-agro-version} format corresponds to a specfic data set with pairs of entries in a given location. 
