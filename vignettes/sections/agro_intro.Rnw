...
\subsection{Decision tree to choose analysis and experimental design regarding agronomic analysis}
...
For each family, a decision tree are divided into five categories (Figure \ref{tree}):
\begin{enumerate}
...
In sections design of experiment and analysis, the branchs of the descision tree concerned are displayed. 
...
\node{ \huge \textbf{Family} }
        child { node {Number of plots per location}
      		child { node {Number of locations years}
...
\textbf{Family 3} gathers one analysis which estimates effects from family 1 and 2. This is for analysis in a network of farms. Environment effect can not be estimated as location, and year are separated.


\subsection{Family 4}

\textbf{Family 4} gathers analysis answering specific research questions. This is for analysis on one farm or more.
...
After deign the experiment and describe the data, each family of analysis is implemented by several analysis with the same workflow :
\begin{enumerate}
\item Run the model
