#' Get dataframe for factors' variation for each variable from a list whose elements are output from \code{\link{check_model.fit_model_GxE}}
#' 
#' @description 
#' \code{post_hoc_variation_model_GxE} returns a dataframe for factors' variation for each variable from a list whose elements are output from \code{\link{check_model.fit_model_GxE}}
#'
...
#' @export
#'
post_hoc_variation_model_GxE = function(list_out_check_model){
    ...
    for(i in 1:length(list_out_check_model)){
      ...
      variable = list_out_check_model[[i]]$model_GxE$info$variable
