...
3.**install several dependencies** When installing the package, `R` will load several package and may ask you to download dependencies. For exemple `JAGS` software in order to do bayesian analysis (http://mcmc-jags.sourceforge.net/).

4.**download the current version of the package**: [download v0.25](https://github.com/priviere/PPBstats/archive/v0.25.zip)

5.**unzip the folder** on your local directory 

6.**install the package** thanks to the `install` funtion from the `devtool` package 
```{r, eval=FALSE}
devtools::install("PPBstats-0.25")
```
