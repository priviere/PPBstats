...
\begin{tikzpicture}
  ...
  \node [, below=of GEBIM.east, left, yshift=-4ex] (DDN) {describe\_data.data\_agro};
  \node [, below=of DDN.east, left] (DDA) {describe\_data.data\_network};

  \draw [-, decorate,decoration=brace] (DDN.north east) -- (DDA.south east)
    node [midway, virtual, xshift=-4pt] (X0) {};
  \node [several-all, right=of X0] (DD) {describe\_data};
  \draw (X0) to (DD);


  %% check_model
  
  \node [m1, below=of DDA.east, left, yshift=-4ex] (cmm1) {check\_model.fit\_model\_1};
