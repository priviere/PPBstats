...
link-citations: yes
github-repo: priviere/PPBstats_book
...
You're welcome to contribute: see [here](https://github.com/priviere/PPBstats/releases/download/v0.22/contribute_PPBstats.pdf) for more details.
...
## Authors of the package and affiliation {-}
Pierre Rivière^1,2^
Gaelle Van Frank^2^
Olivier David^3^
Facundo Muñoz^4^
...
## Contributions for the package and the book {-}
PR coordinates the package development, wrote the R functions and the present book

GVF test the package and updated the code regarding Sections \@ref(ammi), \@ref(gge), \@ref(model-1), \@ref(model-2) and \@ref(variance-intra)

OD wrote the `JAGS` code and reviewed the R code and the book regarding Sections \@ref(model-1), \@ref(model-2), \@ref(model-1) and \@ref(variance-intra).

FM reformated and improved all the code regarding S3 methods and improved the book by formating figure with `Tikz`.
