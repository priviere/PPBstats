...
data_to_map = droplevels(unique(data_network_unipart_sl[,c("location_parent", "long_parent", "lat_parent")]))
colnames(data_to_map) = c("location", "long", "lat")
```
...
p = plot.data_agro(
  data_model_GxE, data_to_map,
  plot_type = "map", labels_on = "location"
)
p$map
```
...
```{r,warning=FALSE,message=FALSE}
p = plot.data_agro(
  data_model_GxE, data_to_map, vec_variables = c("y1", "desease"),
  plot_type = "map"
)
p$pies_on_map_y1
